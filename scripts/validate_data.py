...
class Interaction(BaseModel):
    ...
    compound1_id: str = Field(..., min_length=1)
    compound2_id: str = Field(..., min_length=1)
...
class DataValidator:
    ...
    def validate_all(self) -> bool:
        ...
        # Validate CSV files
        self.validate_csv('compounds.csv', Compound, self.compounds)
        self.validate_csv('interactions.csv', Interaction, self.interactions)
        self.validate_csv('sources.csv', Source, self.sources)
        
        # Validate JSON files
        self.validate_json('compounds.json', 'compound_id')
        self.validate_json('interactions.json', 'interaction_id')
        self.validate_json('sources.json', 'source_id')
    ...
    def validate_csv(self, filename: str, model: type[BaseModel], storage: Dict) -> None:
        ...
        try:
            with open(filepath, 'r', encoding='utf-8') as f:
                ...
                id_field = list(model.model_fields.keys())[0]  # First field is ID
                ...
                for row_num, row in enumerate(rows, start=2):  # Start at 2 (header is 1)
                    ...
                    try:
                        validated = model(**row)
                        row_id = getattr(validated, id_field)
    ...
    def check_referential_integrity(self) -> None:
        ...
        for interaction_id, interaction in self.interactions.items():
            compound1_id = interaction.get('compound1_id')
            compound2_id = interaction.get('compound2_id')
            
            if compound1_id and compound1_id not in self.compounds:
                self.errors.append(
                    f"interactions.csv:{interaction_id}: References unknown compound1_id '{compound1_id}'"
                )
            
            if compound2_id and compound2_id not in self.compounds:
                self.errors.append(
                    f"interactions.csv:{interaction_id}: References unknown compound2_id '{compound2_id}'"
                )
